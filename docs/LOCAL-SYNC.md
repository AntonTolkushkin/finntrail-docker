...
8. Файлам назначаются локальные development-права, чтобы PHP и Bitrix могли
   изменять структуру сайта, загружать файлы и устанавливать обновления.
9. Запускаются `mysql`, `redis`, `mailpit`, `php`, `nginx`. Контейнер `cron`
   остаётся остановленным, чтобы локальная копия не выполняла
   production-агенты.
...
Письма локального сайта перехватывает Mailpit. Они не уходят реальным
получателям и доступны в браузере по адресу <http://127.0.0.1:8025/>.
